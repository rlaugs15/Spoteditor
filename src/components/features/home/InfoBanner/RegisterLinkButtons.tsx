...
export default function RegisterLinkButtons({ label }: RegisterLinkButtonsProps) {
  return (
    <div className="flex items-center">
      <Button
        className="rounded-full w-56 h-15 !text-text-sm web:!text-text-md font-medium bg-white text-black hover:bg-white hover:text-black"
        size="lg"
        asChild
      >
        <Link href={REGISTER_PATHS.MOOD}>{label}</Link>
      </Button>
      <Button className="rounded-full w-15 h-15 bg-white hover:bg-white" size="icon" asChild>
